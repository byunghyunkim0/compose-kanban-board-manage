...
비즈니스 로직
- [ ] 상태별 전이 규칙이 올바르게 동작하는지 검증 (허용되는 경로 vs 차단되는 경로)
- [ ] 상태별 제약 사항(담당자 필수 여부, 삭제 가능 여부) 검증
- [ ] KanbanStatus에 Review 상태 추가
- [ ] 상태별 삭제 가능 여부 및 담당자 필수 여부 검증 로직 구현
- [ ] 상태 전이 규칙 로직 구현

UI 및 인터랙션
- [ ] 태스크 카드 클릭 시 수정 모달 진입
- [ ] 상태 변경 시 전이 규칙에 어긋나는 경우 스낵바 표시
- [ ] 수정/삭제 완료 후 스낵바 표시
- [ ] 삭제 버튼 클릭 시 상태에 따라 버튼 활성화/비활성화 또는 삭제 동작 확인
- [ ] 태스크 수정 후 정보가 정상적으로 반영되는지 확인
